...
pub struct Args {
    #[clap(
        short,
        long,
        help = "Path to the directory containing the files hawk will analyze. The default directory is `./contracts`"
    )]
    ...
    #[clap(
        short,
        long,
        help = "Path to the toml file containing the hawk configuration when not using the default settings."
    )]
...
impl Opts {
    pub fn new() -> Opts {
        ...
        let (optimizations, vulnerabilities, qa) = if args.toml.is_some() {
            ...
            let hawk_toml: hawkToml =
                toml::from_str(&toml_str).expect("Could not convert toml contents to hawkToml");
...
If the `--path` flag is not passed, hawk will look for `./contracts` by default.
